...
export const asssignElectionPositionsEpic = (action$) =>
    action$.pipe(
        ofType("ASSIGN_ELECTION_POSITIONS_ATTEMPT"),
        mergeMap((action) =>
            from(api.hitApi(`/election/${action.payload.electionId}/assign-position`, action.payload, "POST")).pipe(
                mergeMap((response) => {
                    if (action.cb) action.cb(null, response?.data);
                    return of({
                        type: "ASSIGN_ELECTION_POSITIONS_SUCCESS",
                        payload: response.data
                    }, {
                        type: "VIEW_ELECTION_LIST_ATTEMPT"
                    });
                ...
                catchError((err) => {
                    if (action.cb) action.cb(err);
                    return of({
                        type: "ASSIGN_ELECTION_POSITIONS_FAILURE",
                        payload: err.response
                    });
                })
            )
...
export const viewAsssignElectionPositionsEpic = (action$) =>
    action$.pipe(
        ofType("VIEW_ASSIGN_ELECTION_POSITIONS_ATTEMPT"),
        mergeMap((action) =>
            from(api.hitApi(`/election/${action.payload.electionId}/positions`, {}, "GET")).pipe(
                mergeMap((response) => {
                    if (action.cb) action.cb(null, response?.data);
                    return of({
                        type: "VIEW_ASSIGN_ELECTION_POSITIONS_SUCCESS",
                        payload: response.data
                    });
                }),
                catchError((err) => {
                    if (action.cb) action.cb(err);
                    return of({
                        type: "VIEW_ASSIGN_ELECTION_POSITIONS_FAILURE",
                        payload: err.response
                    });
                })
            )
