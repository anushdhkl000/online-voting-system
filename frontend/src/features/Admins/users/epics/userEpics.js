...
export const verifyUserDetailsEpic = (action$) =>
    action$.pipe(
        ofType("VERIFY_USER_DETAILS_ATTEMPT"),
        mergeMap((action) =>
            from(api.hitApi(`/auth/user/verify-details/${action.userId}`, { verify: action.verify }, "PUT")).pipe(
                mergeMap((response) => {
                    if (action.cb) action.cb(null, response?.data);
                    return of({
                        type: "VERIFY_USER_DETAILS_SUCCESS",
                        payload: response.data
                    },
                        {
                            type: "VIEW_USER_LIST_ATTEMPT",
                ...
                catchError((err) => {
                    if (action.cb) action.cb(err);
                    return of({
                        type: "VERIFY_USER_DETAILS_FAILURE",
                        payload: err.response
                    });
                })
            )
